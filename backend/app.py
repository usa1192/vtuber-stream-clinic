...
コメント対応率は扱いません。silenceRate はバックエンドの音声解析で後から入れるため、推測しないでください。
...
def get_audio_stream_url(youtube_url: str) -> str:
    try:
        ...
        raise RuntimeError("yt-dlp が未インストールです。start_windows.batを再起動してください。") from exc
    ...
    def audio_rank(item):
        return (
            1 if item.get("ext") in {"m4a", "webm"} else 0,
            float(item.get("abr") or 0),
            float(item.get("tbr") or 0),
        )

    audio_formats.sort(key=audio_rank, reverse=True)
...
def get_ffmpeg_exe() -> str:
    try:
        ...
        raise RuntimeError("ffmpeg を準備できませんでした。start_windows.batを再起動してください。") from exc
...
def analyze_with_retries(client: genai.Client, youtube_url: str, analysis_mode: str) -> AnalysisResult:
    ...
    try:
        ...
        result.warnings.append("映像の詳細は未確認です。無言率は別途、簡易音声解析で補います。")
        return result
