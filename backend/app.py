...
class AnalysisResult(BaseModel):
    ...
    highlights: List[Highlight] = Field(min_length=3, max_length=6)
...
ANALYSIS_SCHEMA = {
    ...
    "properties": {
        ...
        "mainContentStartedAt": {"type": "string"},
...
分析方針:
- このサービスは「推しの配信の見どころチェック」を主目的にする。欠点探しではなく、ファンが見返したくなる瞬間を発見する
...
- mainContentStartedAt は「本編開始の推定時刻」として、HH:MM:SS または MM:SS 形式にする
- 本編開始の推定は、配信ジャンルに合わせて判断する。ゲーム配信なら雑談や告知が一区切りつき、ゲーム画面・メニュー・操作画面・ロード画面へ移る瞬間や「始めよう」などの発話を重視する
- 歌枠なら1曲目の伴奏や歌唱開始、企画配信ならルール説明後の1問目/1戦目/1ターン目、雑談配信なら挨拶後に最初の主題へ入る瞬間を本編開始とする
- 最初から本編が始まっている場合は 00:00 とする
- highlights は推しの見どころ候補を6件返す。十分な根拠がない場合でも最低3件は返す
- highlights は「注目シーン」3件と「追加の見どころ」3件のつもりで、時間帯が偏りすぎないように選ぶ
- ゲーム配信では、ボス戦、強敵戦、初見リアクション、ガチャやレアドロップ、ミスややらかし、逆転、クリアや達成、新エリアや新要素の発見、コメントと盛り上がった場面、配信者のツッコミや感情が大きく動いた場面を重視する
- 歌枠では歌い出し、サビ、感情が乗った場面、意外な選曲、MCとのつながりを重視する
- 雑談や企画では、話題が跳ねた瞬間、笑いが起きた場面、印象的な言い回し、企画の転換点を重視する
...
- highlights の reason は、なぜファンが見返したくなる見どころなのかを40〜90字程度で具体的に書く
- コメント対応率は扱わない
- silenceRate は互換性のために残すが、値は推測しない
- warnings には「Gemini推定値」「公開YouTube動画のみ対応」「分析モード: {mode["label"]}」などの注意を含める

JSONキー:
streamType, overallScore, summary, mainTopicStartedAt, mainContentStartedAt, titleMatchScore, talkDensityScore, highlights, advice, goodPoints, dropOffPoints, nextActions, analysisMode, warnings
""".strip()
...
def build_transcript_prompt(transcript: str) -> str:
    return f"""
あなたはVTuber配信の見どころを発見する「配信カルテ」の分析エンジンです。
...
mainContentStartedAt は「本編開始の推定時刻」として、ゲーム配信なら雑談や告知が終わってゲーム画面や操作に移る瞬間、歌枠なら1曲目が始まる瞬間、企画配信なら企画の1問目/1戦目が始まる瞬間、雑談配信なら最初の主題に入る瞬間を根拠に判断してください。
highlights は推しの見どころ候補を6件返してください。字幕だけで6件の根拠が弱い場合でも、最低3件は返してください。
ゲーム配信では、初見リアクション、ボス戦、達成、やらかし、逆転、発見、配信者の感情が動いた発言を重視してください。
コメント対応率は扱いません。silenceRate は互換性のために残しますが、値は推測しないでください。

JSONキー:
streamType, overallScore, summary, mainTopicStartedAt, mainContentStartedAt, titleMatchScore, talkDensityScore, highlights, advice, warnings
...
def normalize_analysis_result(result: AnalysisResult, analysis_mode: str) -> AnalysisResult:
    ...
    if not result.mainContentStartedAt or result.mainContentStartedAt == "--:--":
        result.mainContentStartedAt = result.mainTopicStartedAt
